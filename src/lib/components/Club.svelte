...
<div class="dice-container">
	...
	>
		...
		<!-- Dice dots -->
		{#if diceResult}
			<path d={diceDots[diceResult]} fill="white" />
			{#each diceDots[diceResult].split('M').filter(Boolean) as dot}
				<circle cx={dot.split(' ')[0]} cy={dot.split(' ')[1]} r="5" fill="white" />
			{/each}
		{/if}
	</svg>
	<button class="putt-button" class:locked={dice.locked} onclick={putt}>Putt</button>
</div>
